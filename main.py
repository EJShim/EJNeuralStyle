...
with tf.Graph().as_default(), tf.Session() as sess:
    ...
    content_features = sess.run(nets[CONTENT_LAYER], feed_dict={image:content_image_pre})
...
STYLE_PATH = cwd + "/data/goghStyle.jpg"
...
with tf.Graph().as_default(), tf.Session() as sess:
    ...
    for idx, layer in enumerate(STYLE_LAYERS):
        curr_features = sess.run( nets[layer], feed_dict={image:style_image_pre} )
...
with tf.Graph().as_default(), tf.Session() as sess:
    ...
    for i in range(iterations):
        ...
        if i % 100 == 0 or i == iterations-1:
            ...
            filename =  "output/" + str(i) + "image.jpg"
            scipy.misc.imsave(filename, stylized_img)
